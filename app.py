...
def resolved_font_family(font_name: str | None, font_path: Path | None) -> str:
    """Return the libass family matching the actual resolved font file."""
    if font_path and "NotoSansMyanmar" in font_path.name:
        return "Noto Sans Myanmar"
    if font_path and "Padauk" in font_path.name:
        return "Padauk"
    return subtitle_font_family(font_name)
...
def render_live_subtitle_browser_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, x_percent: int = 50, y_percent: int = 86) -> None:
    ...
    font_size_safe = max(12, min(96, int(font_size)))
    component_height = max(150, min(230, int(frame.height * 0.42)))
    html = f"""
    <style>
      {font_face}
      html,body{{margin:0;background:transparent;overflow:hidden;}}
      .stage{{position:relative;width:100%;height:{component_height}px;overflow:hidden;background:#111;border-radius:10px;}}
      .stage img{{width:100%;height:100%;object-fit:contain;display:block;}}
      .caption{{position:absolute;left:{x}%;top:{y}%;transform:translate(-50%,-50%);max-width:92%;padding:8px 12px;text-align:center;color:{text_color_safe};font-family:'MgMyanmar','Noto Sans Myanmar','Pyidaungsu',sans-serif;font-size:{font_size_safe}px;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:-2px -2px 0 {outline_color_safe},2px -2px 0 {outline_color_safe},-2px 2px 0 {outline_color_safe},2px 2px 0 {outline_color_safe};z-index:2;}}
...
def build_subtitle_filter(srt_path: Path, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, position: str, fonts_dir: Path | None = None, x_percent: int = 50, y_percent: int = 86) -> str:
    ...
    return f"subtitles=filename='{escaped_srt}':charenc=UTF-8{fonts_option}:force_style='{force_style}'"
...
def calculate_sync_plan(video_duration: float, audio_duration: float, audio_speed: float, timing_basis: str = "Audio အချိန်") -> dict[str, float]:
    """Fit media to the user-selected narration or source-video duration basis."""
    ...
    if timing_basis == "Video အချိန်":
        ...
        # Audio basis preserves the chosen narration pace unless it exceeds
        # the source clip, where it safely fits into the available video.
        target = min(source_video, requested_duration)
...
def merge_audio_video(video_path: Path, audio_bytes: bytes, platform: str, speed: float = 1.0, subtitle_srt: str = "", subtitle_font: str = "Noto Sans Myanmar", subtitle_size: int = 34, subtitle_text_color: str = "#FFFFFF", subtitle_outline_color: str = "#000000", subtitle_background_mode: str = "Transparent", subtitle_background_color: str = "#000000", subtitle_background_opacity: int = 55, subtitle_position: str = "Bottom", subtitle_x: int = 50, subtitle_y: int = 86, effect_mirror: bool = False, effect_auto_zoom: bool = False, effect_color_filter: bool = False, effect_pitch_alter: bool = False, effect_background_blur: bool = False, logo_path: str | Path | None = None, logo_position: str = "Top right", logo_motion: str = "Static", moving_logo_text: str = "", text_position: str = "Bottom center", quality_mode: str = "720", timing_basis: str = "Audio အချိန်") -> bytes:
    ...
    if srt_path is not None:
        # The narration may be compressed to fit a shorter source video.  Scale
        # only subtitle timestamps to that final audio duration; audio remains
        # exactly on its existing timeline.
    ...
    active_label = ""
    if complex_filter:
        pre_fit_filter = ",".join(video_parts)
        video_filter = (
            f"[0:v]{pre_fit_filter},split=2[mgk_bg][mgk_fg];"
            f"[mgk_bg]scale={output_width}:{output_height}:force_original_aspect_ratio=increase,crop={output_width}:{output_height},boxblur=20:10[mgk_blur];"
    ...
    if srt_path is not None:
        ...
        subtitle_filter = build_subtitle_filter(srt_path, subtitle_font_name, subtitle_size, subtitle_text_color, subtitle_outline_color, subtitle_background_mode, subtitle_background_color, subtitle_background_opacity, subtitle_position, subtitle_fonts_dir, subtitle_x, subtitle_y)
        if complex_filter:
    ...
    if logo_file:
        ...
        if complex_filter:
            graph = f"{video_filter};[2:v]format=rgba,scale=iw*0.22:-1[logo];[{active_label}][logo]overlay=x={logo_x}:y={logo_y}:shortest=1[vout]"
        else:
            graph = f"[0:v]{video_filter}[base];[2:v]format=rgba,scale=iw*0.22:-1[logo];[base][logo]overlay=x={logo_x}:y={logo_y}:shortest=1[vout]"
...
def run_one_click_youtube_export(video_path: Path, duration_seconds: int, tone: str, mode: str, voice: str, voice_style: str, audio_speed: float, blur_enabled: bool, blur_masks: list[dict], blur_strength: int, blur_background: str, blur_color: str, export_style: dict, logo_path: str | None, logo_position: str, logo_motion: str, moving_logo_text: str, text_position: str, effect_auto_zoom: bool, effect_color_filter: bool, effect_pitch_alter: bool, subtitle_enabled: bool = True, progress_callback=None, timing_basis: str = "Audio အချိန်", effect_background_blur: bool = False) -> tuple[bytes, str, str, bytes]:
    ...
    if bool(st.session_state.get("effect_mirror", False)):
        edited_video = apply_copyright_edit(video_path, True, False, False, False)
    if blur_enabled and blur_masks:
        boxes = [(int(mask["x"]), int(mask["y"]), int(mask["width"]), int(mask["height"])) for mask in blur_masks]
    ...
    generated_srt = normalize_srt_text(script_to_srt(script, adjusted_duration)) if subtitle_enabled else ""
    if progress_callback:
        progress_callback(78, "Video ဖိုင် ပေါင်းနေသည်")
    merged = merge_audio_video(
        edited_video,
        audio,
        st.session_state.get("target_platform", "YouTube"),
        audio_speed,
...
def main():
    ...
    if True:
        right = st.container()
        with right:
            st.subheader("1 · Video ပြင်ဆင်ရန်")
            ...
            if False and st.button("Script ရေးမယ်", type="primary", use_container_width=True):
                if not duration_valid:
                    st.warning("အရင်ဆုံး Recap အရှည်ကို မှန်ကန်အောင် ထည့်ပါ။")
                    st.stop()
                progress_message = "Video ကို AI က သုံးသပ်ပြီး ဘာသာပြန်နေပါတယ်..." if mode == "Faithful full translation" else "AI က Script ရေးနေပါတယ်..."
                with st.status(progress_message, expanded=True) as translation_status:
                    try:
                        if mode == "Faithful full translation":
                            translation_status.write("Video ထဲက အသံကို ခွဲပြီး စာသားအကြောင်းအရာကို လုပ်ဆောင်နေပါတယ်... ဖိုင်အရွယ်အစားသေးလို့ ပိုမြန်ပါမယ်။")
                        else:
                            translation_status.write("Video အကြောင်းအရာကို AI က လေ့လာနေပါတယ်...")
        ...
        if False and st.button("Video ထုတ်မယ် →", type="primary", use_container_width=True, key="one-click-youtube-export"):
            ...
            with st.status("Video အားလုံးကို အစဉ်လိုက်ထုတ်နေပါတယ်...", expanded=True) as pipeline_status:
                try:
                    ...
                    st.error(f"Video export မအောင်မြင်ပါ: {exc}")
    ...
    if True:
        ...
        logo_col, text_col = st.columns(2, gap="small")
        with logo_col:
            st.caption("ပုံ Logo")
        ...
        with text_col:
            st.caption("စာသား Logo")
            moving_logo_text = st.text_input("Logo စာသား", key="moving_logo_text", placeholder="စာပဲထည့်လည်းရပါတယ်", label_visibility="collapsed")
            st.session_state.setdefault("logo_motion", "Left static")
            logo_motion = st.selectbox("စာသားပုံစံ", ["Left static", "Right static", "Full-screen movement"], format_func=lambda value: {"Left static": "ဘယ်ငြိမ်", "Right static": "ညာငြိမ်", "Full-screen movement": "အနှံ့ပြေး"}[value], key="logo_motion")
        ...
        st.caption("ရွေးထားတဲ့ Effect နဲ့ Logo ကို Final Video export အချိန်မှာ ထည့်ပေးပါမယ်။")
    ...
    if True:
        ...
        timing_basis = st.radio("အချိန်ယူမယ်", ["Audio အချိန်", "Video အချိန်"], horizontal=True, key="timing_basis")
    ...
    if True:
        ...
        if subtitle_enabled:
            ...
            if generated_srt and not st.session_state.get("subtitle_srt_editor", "").strip():
                st.session_state["subtitle_srt_editor"] = generated_srt
            st.caption("စာတန်းထိုး ချိန်ရန်")
            subtitle_controls = st.container()
            with subtitle_controls:
                font_options = list(FONT_FILES.keys())
                if st.session_state.get("subtitle_font") not in font_options:
                    st.session_state["subtitle_font"] = font_options[0]
                style_row_one = st.columns(3, gap="small")
                with style_row_one[0]:
                    subtitle_font = st.selectbox("Font", font_options, key="subtitle_font")
                with style_row_one[1]:
                    subtitle_size = st.slider("Size", 16, 72, 34, key="subtitle_size")
                with style_row_one[2]:
                    subtitle_background_mode = st.selectbox("Background", ["Transparent", "Solid background"], key="subtitle_background_mode")
                style_row_two = st.columns(3, gap="small")
                with style_row_two[0]:
                    subtitle_text_color = st.color_picker("စာအရောင်", "#FFFFFF", key="subtitle_text_color")
                with style_row_two[1]:
                    subtitle_outline_color = st.color_picker("Outline", "#000000", key="subtitle_outline_color")
                with style_row_two[2]:
                    subtitle_background_color = st.color_picker("နောက်ခံ", "#000000", key="subtitle_background_color")
                style_row_three = st.columns(3, gap="small")
                with style_row_three[0]:
                    subtitle_background_opacity = st.slider("Opacity", 0, 100, 55, key="subtitle_background_opacity")
                with style_row_three[1]:
                    subtitle_x = st.slider("X", 0, 100, int(st.session_state.get("subtitle_x", 50)), key="subtitle_x")
                with style_row_three[2]:
                    subtitle_y = st.slider("Y", 0, 100, int(st.session_state.get("subtitle_y", 86)), key="subtitle_y")
                subtitle_position = "Bottom"
                st.session_state["subtitle_export_font"] = subtitle_font
                # Snapshot the exact values rendered in the lower preview. The
                # export button below must never fall back to an older widget state.
            ...
            preview_action, preview_hint = st.columns([1, 2], gap="small")
            with preview_action:
                refresh_test_side = st.button("Test Side", key="subtitle-test-side", use_container_width=True)
            with preview_hint:
                st.caption("X/Y ကိုပြောင်းပြီး Test Side နှိပ်ပါ")
            subtitle_text = preview_caption_text(generated_srt) or "မြန်မာစာတန်းထိုး စမ်းသပ်ခြင်း"
            try:
                subtitle_test_source = st.session_state.get("blurred_video_path") or st.session_state.get("copyright_video_path") or st.session_state.get("video_path")
                if subtitle_test_source:
                    subtitle_test_path = Path(subtitle_test_source)
                    if refresh_test_side:
                        cached_subtitle_preview_frame.clear()
                    subtitle_test_frame = cached_subtitle_preview_frame(str(subtitle_test_path), subtitle_test_path.stat().st_mtime_ns)
                    render_live_subtitle_browser_preview(subtitle_test_frame, subtitle_text, subtitle_font, subtitle_size, subtitle_text_color, subtitle_outline_color, subtitle_background_mode, subtitle_background_color, subtitle_background_opacity, subtitle_x, subtitle_y)
    ...
    if True:
        ...
        if st.button("Video ထုတ်မယ် →", type="primary", use_container_width=True, key="final-one-click-export"):
            ...
            with st.status("Video အားလုံးကို အစဉ်လိုက်ထုတ်နေပါတယ်...", expanded=True) as pipeline_status:
                try:
                    ...
                    st.success("Video + အသံ + Logo + Effect အားလုံး ပြီးပါပြီ" + (" + မြန်မာစာတန်းထိုး" if generated_srt else "") + "။")
                    st.rerun()
                except Exception as exc:
                    pipeline_status.update(label="Export မအောင်မြင်ပါ", state="error", expanded=True)
                    st.error(f"Video export မအောင်မြင်ပါ: {exc}")
        ...
        final_srt_text = st.session_state.get("generated_srt", "") if st.session_state.get("subtitle_enabled", True) else ""
        if final_video_data:
            ...
            if isinstance(final_video_data, (bytes, memoryview)):
                with st.container(border=True):
                    ...
                    st.markdown("<div class='final-result-copy'>ဒီနေရာမှာပဲ ကြည့်ပြီး ဖိုင် ၃ ခုကို သီးသန့်ဒေါင်းပါ။</div>", unsafe_allow_html=True)
                    st.video(final_video_data)
                    st.download_button("Final Video ဒေါင်းရန်", final_video_data, file_name=f"movie-recap-{st.session_state.get('target_platform', 'video').lower()}.mp4", mime="video/mp4")
