...
def main():
    ...
    if active_step == 1:
        st.divider()
        st.subheader("အကုန်ရွေးပြီး တစ်ခါတည်းထုတ်ရန်")
        st.caption("Script → Effect → Blur → Voiceover + SRT → Logo + Subtitle → YouTube Video")
        if st.button("YouTube Video တစ်ခါတည်းထုတ်မယ် →", type="primary", use_container_width=True, key="one-click-youtube-export"):
