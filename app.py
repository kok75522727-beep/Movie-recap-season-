...
def render_menu() -> None:
    with st.popover("⋮", use_container_width=True):
        ...
        st.caption("API Key ကို Menu ထဲမှာပဲ ထည့်ပါ။ Session အတွင်းသာ အသုံးပြုထားပြီး GitHub/URL ထဲ မသိမ်းပါ။")
        key = st.text_input("Google AI Studio API Key", type="password", value=st.session_state.get("google_ai_key", ""), placeholder="AIza...", key="menu_google_ai_key")
        if key.strip():
...
def merge_audio_video(video_path: Path, audio_bytes: bytes, platform: str, speed: float = 1.0) -> bytes:
    ...
    effective_video_duration = video_duration / speed
    audio_duration = max(0.1, len(audio_bytes) / (24000 * 2))
    # Apply the same selected speed to both the source video and the voiceover.
    audio_filter = [build_atempo_filter(speed)]
    speed_adjusted_audio_duration = audio_duration / speed
    if speed_adjusted_audio_duration > effective_video_duration + 0.25:
        # Only correct the remaining duration difference; never clip the narration ending.
        audio_filter.append(build_atempo_filter(speed_adjusted_audio_duration / effective_video_duration))
    elif speed_adjusted_audio_duration < effective_video_duration - 0.25:
        # Keep the selected speed and pad silence to the video end.
        audio_filter.append("apad")
    audio_filter.append("aresample=async=1:first_pts=0")
    preset = PLATFORM_OPTIONS.get(platform, PLATFORM_OPTIONS["YouTube"])
    video_filter = f"setpts=PTS/{speed:.6f},scale={preset['width']}:{preset['height']}:force_original_aspect_ratio=increase,crop={preset['width']}:{preset['height']}"
    command = [
        "ffmpeg", "-y", "-i", str(video_path), "-i", str(audio_path),
        "-map", "0:v:0", "-map", "1:a:0", "-vf", video_filter, "-c:v", "libx264", "-preset", "medium", "-crf", "18",
        "-c:a", "aac", "-b:a", "192k", "-af", ",".join(audio_filter),
        "-t", str(effective_video_duration), "-movflags", "+faststart", str(output_path),
    ]
    try:
...
def main():
    ...
    if st.session_state.get("script"):
        ...
        try:
            ...
            frame_times = sampled_frame_times(duration_for_frames) or [0.0]
            selected_frame_time = st.select_slider("စာတန်းအများဆုံး/အရှည်ဆုံးပေါ်တဲ့ Frame ရွေးပါ", options=frame_times, value=frame_times[0], format_func=lambda value: f"{format_duration(round(value))} မှာ Frame")
            ...
            original_width, original_height = dimensions or (preview_frame.width, preview_frame.height)
        ...
        st.divider()
        st.subheader("4 · Video + အသံ Speed တူတူချိန်ပြီး Gemini အသံ")
        speed_label = st.selectbox("Video + Voiceover Speed", ["0.5×", "0.75×", "1×", "1.25×", "1.5×", "2×"], index=2, key="video_speed")
        speed = float(speed_label.replace("×", ""))
        ...
        if voiceover_duration and video_duration:
            adjusted_audio_duration = voiceover_duration / speed
            adjusted_video_duration = video_duration / speed
            st.info(f"Video {format_duration(video_duration)} → {format_duration(round(adjusted_video_duration))} · အသံ {format_duration(round(voiceover_duration))} → {format_duration(round(adjusted_audio_duration))} · နှစ်ခုလုံး {speed_label}")
        else:
            st.caption("Voiceover ထွက်ပြီးနောက် Video နဲ့ အသံကို ရွေးထားတဲ့ Speed တူတူနဲ့ ချိန်ပြီး အရှည်ကို ပြပါမယ်။")
    ...
    if st.session_state.get("audio"):
        ...
        if st.button("Video + Voiceover ဖိုင် ထုတ်မယ်", use_container_width=True):
            with st.spinner("Video နဲ့ အသံကို ရွေးထားတဲ့ Speed တူတူနဲ့ Sync လုပ်နေပါတယ်..."):
                try:
                    source_video = Path(st.session_state.get("blurred_video_path") or st.session_state.video_path)
                    st.session_state.output_video = merge_audio_video(source_video, st.session_state.audio, platform, speed)
                    register_generation()
                    st.success("Video နဲ့ Voiceover ကို အရှည်ကိုက်အောင်၊ အသံမပြတ်ဘဲ Quality ထိန်းပြီး ထုတ်ပြီးပါပြီ။")
                except Exception as exc:
