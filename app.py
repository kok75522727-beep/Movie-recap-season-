...
def normalize_srt_text(srt_text: str) -> str:
    ...
    for raw_block in re.split(r"\n\s*\n", cleaned):
        ...
        try:
            ...
            text = wrap_subtitle_lines(" ".join(lines[2:]), SUBTITLE_MAX_LINE_CHARS)
            if not text:
                continue
...
def render_live_subtitle_browser_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, x_percent: int = 50, y_percent: int = 86) -> None:
    ...
    clean_text = preview_caption_text(text, SUBTITLE_MAX_LINE_CHARS) if "-->" in (text or "") else "\n".join(wrap_subtitle_lines(" ".join((text or "").split()), SUBTITLE_MAX_LINE_CHARS)[:2])
...
def calculate_sync_plan(video_duration: float, audio_duration: float, audio_speed: float) -> dict[str, float]:
    """Fit narration and video to one bounded duration; never extend past the source video."""
