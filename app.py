...
st.set_page_config(page_title="Mg Khant · Movie Recap", page_icon="🎬", layout="centered", initial_sidebar_state="collapsed")
...
def render_menu() -> None:
    with st.popover("⚙️ Setting", use_container_width=True):
        ...
        # Keep administrator controls outside the public application UI.
...
def apply_cinematic_theme():
    st.markdown(
        ...
        @media (max-width:900px) {
            ...
            div[data-testid='stHorizontalBlock']:has(.mgk-app-brand) { flex-direction:row !important; flex-wrap:nowrap !important; align-items:center !important; }
            div[data-testid='stHorizontalBlock']:has(.mgk-app-brand) > div[data-testid='column'] { min-width:0 !important; width:auto !important; flex:0 1 auto !important; }
            div[data-testid='stHorizontalBlock']:has(.mgk-app-brand) > div[data-testid='column']:first-child { flex:1 1 auto !important; }
...
def main():
    ...
    brand_col, settings_col = st.columns([0.72, 0.28], gap="small")
    with brand_col:
    ...
    with st.container(border=True):
        ...
        setup_row_one = st.columns(2, gap="small")
    ...
    with st.container(border=True):
        ...
        for position, (effect_key, effect_label, effect_hint) in enumerate(effect_steps):
            target_col = effect_columns[position % 3]
            with target_col:
                with st.container(border=True):
