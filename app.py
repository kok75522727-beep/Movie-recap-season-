    ...
    with st.popover("☰ Menu", use_container_width=True):
        ...
        with settings_right:
            if st.button("Test Gemini API", use_container_width=True, key="menu_test_api"):
                try:
                    result = get_client().interactions.create(model=st.session_state.gemini_text_model, input="Reply with the single word: READY")
...
def extract_preview_frame(video_path: Path, at_seconds: float = 0) -> Image.Image:
    ...
    if not result.stdout:
        raise RuntimeError("Video frame မရပါ။ Video format ကို စစ်ပါ။")
...
def merge_audio_video(video_path: Path, audio_bytes: bytes, platform: str, speed: float = 1.0) -> bytes:
    ...
    audio_filter = []
    if audio_duration > effective_video_duration + 0.25:
        # Narration is longer: speed it up instead of clipping the ending.
        audio_filter.append(build_atempo_filter(audio_duration / effective_video_duration))
    elif audio_duration < effective_video_duration - 0.25:
        # Narration is shorter: keep its natural speed and pad silence to the video end.
...
def main():
    ...
    with right:
        ...
        speed_label = st.selectbox("Video Speed", ["0.5×", "0.75×", "1×", "1.25×", "1.5×", "2×"], index=2, key="video_speed")
        video_speed = float(speed_label.replace("×", ""))
        adjusted_duration = video_duration / video_speed if video_duration else None
        st.info(f"{platform} အတွက် {platform_preset['ratio']} · {platform_preset['width']}×{platform_preset['height']} · Speed {speed_label}" + (f" · အရှည် {format_duration(round(adjusted_duration))}" if adjusted_duration else ""))
    ...
    if st.session_state.get("script"):
        ...
        try:
            ...
            frame_times = sampled_frame_times(duration_for_frames)
            ...
            if not original_width or not original_height:
                raise RuntimeError("Video width/height metadata မရပါ။ MP4 သို့မဟုတ် WEBM ပြောင်းပြီး ထပ်တင်ပါ။")
    ...
    if st.session_state.get("audio"):
        ...
        if st.button("Video + Voiceover ဖိုင် ထုတ်မယ်", use_container_width=True):
            with st.spinner("အသံနဲ့ Video အရှည်ကို တွက်ပြီး Speed/Sync ချိန်နေပါတယ်..."):
                try:
                    source_video = Path(st.session_state.get("blurred_video_path") or st.session_state.video_path)
                    st.session_state.output_video = merge_audio_video(source_video, st.session_state.audio, platform, video_speed)
                    register_generation()
