...
def merge_audio_video(video_path: Path, audio_bytes: bytes, platform: str, speed: float = 1.0, subtitle_srt: str = "", subtitle_font: str = "Noto Sans Myanmar", subtitle_size: int = 34, subtitle_text_color: str = "#FFFFFF", subtitle_outline_color: str = "#000000", subtitle_background_mode: str = "Transparent", subtitle_background_color: str = "#000000", subtitle_background_opacity: int = 55, subtitle_position: str = "Bottom", subtitle_x: int = 50, subtitle_y: int = 86, effect_mirror: bool = False, effect_auto_zoom: bool = False, effect_color_filter: bool = False, effect_pitch_alter: bool = False, effect_background_blur: bool = False, effect_freeze_bypass: bool = False, logo_path: str | Path | None = None, logo_position: str = "Top right", logo_motion: str = "Static", moving_logo_text: str = "", text_position: str = "Bottom center", quality_mode: str = "720", timing_basis: str = "Audio အချိန်") -> bytes:
    ...
    active_label = ""
    if complex_filter:
        content_crop = detect_content_crop(video_path)
    ...
    if srt_path is not None:
        ...
        # Keep a known Unicode Myanmar fallback in the same libass fontsdir.
        # This prevents square glyphs if a selected display font lacks a rare
        # Burmese codepoint on the deployed FFmpeg/fontconfig environment.
        unicode_fallback = resolve_myanmar_font("ပြည်ထောင်စု 2.5.3 Bold")
        if unicode_fallback and unicode_fallback.is_file() and unicode_fallback.resolve() != myanmar_font_path.resolve():
            shutil.copy2(unicode_fallback, burnin_font_dir / unicode_fallback.name)
    ...
    if True:
        ...
        if subtitle_enabled:
            ...
            if generated_srt and not st.session_state.get("subtitle_srt_editor", "").strip():
                st.session_state["subtitle_srt_editor"] = generated_srt
            subtitle_style_row = st.columns(3, gap="small")
            with subtitle_style_row[0]:
                subtitle_font = st.selectbox("Font", list(FONT_FILES.keys()), key="subtitle_font")
            with subtitle_style_row[1]:
                subtitle_text_color = st.color_picker("စာအရောင်", "#FFFFFF", key="subtitle_text_color")
            with subtitle_style_row[2]:
                subtitle_background_color = st.color_picker("Background", "#000000", key="subtitle_background_color")
            subtitle_test_row = st.columns(3, gap="small")
            with subtitle_test_row[0]:
                subtitle_x = st.slider("X", 0, 100, int(st.session_state.get("subtitle_x", 50)), key="subtitle_x")
            with subtitle_test_row[1]:
                subtitle_y = st.slider("Y", 0, 100, int(st.session_state.get("subtitle_y", 78)), key="subtitle_y")
            with subtitle_test_row[2]:
                show_subtitle_test = st.button("Test Side", use_container_width=True, key="subtitle-test-side")
            if show_subtitle_test:
                st.session_state["subtitle_test_visible"] = True
            ...
            if st.session_state.get("subtitle_test_visible", False):
                try:
                    subtitle_test_source = st.session_state.get("blurred_video_path") or st.session_state.get("video_path")
                    if subtitle_test_source:
                        ...
                        subtitle_test_text = preview_caption_text(generated_srt) or "မြန်မာစာတန်းထိုး စမ်းသပ်ခြင်း"
                        render_live_subtitle_browser_preview(subtitle_test_frame, subtitle_test_text, subtitle_font, 34, subtitle_text_color, "#000000", "Solid background", subtitle_background_color, 60, subtitle_x, subtitle_y)
