...
def render_live_subtitle_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, position: str, x_percent: int = 50, y_percent: int | None = None) -> Image.Image:
    ...
    font_path = Path(__file__).resolve().parent / "fonts" / "ဧက၀၇-Bold.ttf"
    try:
        font = ImageFont.truetype(str(font_path), max(12, int(font_size))) if font_path.exists() else ImageFont.truetype("DejaVuSans.ttf", max(12, int(font_size)))
    except OSError:
    ...
    try:
        ...
        if result.returncode != 0 or not output_path.exists():
            ...
            if subtitle_open_failed:
                # Keep the export usable even when libass rejects a temporary SRT path.
                output_path.unlink(missing_ok=True)
                fallback_command = list(command)
                vf_index = fallback_command.index("-vf") + 1
                fallback_command[vf_index] = base_video_filter
                fallback_result = subprocess.run(fallback_command, capture_output=True, text=True, timeout=1200)
                if fallback_result.returncode == 0 and output_path.exists():
                    return output_path.read_bytes()
                raise RuntimeError(fallback_result.stderr[-1400:] or result.stderr[-1400:])
...
def main():
    ...
    if st.session_state.get("audio"):
        ...
        if subtitle_enabled:
            st.subheader("မြန်မာစာတန်းထိုးနှင့် Style")
            ...
            subtitle_font = st.selectbox("Font", ["A ka 07 · ဧက၀၇-Bold", "Noto Sans Myanmar", "Pyidaungsu", "Myanmar Text", "Noto Sans", "Arial"], key="subtitle_font")
            ...
            subtitle_duration = preview_duration
            subtitle_srt = normalize_srt_text(script_to_srt(subtitle_text, subtitle_duration))
            st.download_button("SRT ဒေါင်းရန်", subtitle_srt.encode("utf-8"), file_name="burmese-subtitles.srt", mime="text/plain", use_container_width=True)
            ...
            with st.spinner("Audio Speed ချိန်ပြီး Video ကို အရှည်ကိုက်အောင် Auto-fit လုပ်နေပါတယ်..."):
                try:
                    ...
                    if st.session_state.get("subtitle_enabled", False) and st.session_state.get("subtitle_text", "").strip():
                        ...
                        subtitle_srt_to_burn = normalize_srt_text(script_to_srt(st.session_state.subtitle_text, max(1, round(shared_final_duration))))
