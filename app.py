...
def render_live_subtitle_browser_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, x_percent: int = 50, y_percent: int = 86) -> None:
    ...
    clean_text = srt_to_plain_text(text) if "-->" in (text or "") else " ".join((text or "").split())
    clean_text = clean_text[:120] or "မြန်မာစာတန်းထိုး စမ်းသပ်ခြင်း"
    wrapped_lines = wrap_subtitle_lines(clean_text, 34)[:2]
    clean_text = html_lib.escape("\n".join(wrapped_lines)).replace("\n", "<br>")
            ...
            with st.spinner("Audio Speed ချိန်ပြီး Video ကို အရှည်ကိုက်အောင် Auto-fit လုပ်နေပါတယ်..."):
                try:
                    ...
                    if st.session_state.get("subtitle_enabled", False) and st.session_state.get("subtitle_text", "").strip():
                        ...
                        subtitle_srt_to_burn = normalize_srt_text(subtitle_srt_to_burn)
