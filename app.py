...
def generate_ai_thumbnail(script: str, aspect_ratio: str, part_label: str | None = None) -> tuple[bytes, str]:
    ...
    interaction = call_gemini(lambda client: client.interactions.create(
        ...
        input=prompt,
        response_format={"type": "image", "mime_type": "image/png", "aspect_ratio": aspect_ratio, "image_size": "1K"},
    ))
...
def apply_cinematic_theme():
    st.markdown(
        ...
        @media (max-width:700px) {
            [data-testid='stFileUploader'] { min-height:0 !important; padding:.16rem .24rem !important; border-radius:7px !important; box-shadow:none !important; background:rgba(18,25,46,.7) !important; }
            [data-testid='stFileUploaderDropzone'] { min-height:0 !important; padding:.22rem .3rem !important; border-radius:6px !important; background:transparent !important; border-style:dashed !important; }
            ...
            [data-testid='stAlert'] { padding:.24rem .34rem !important; border-radius:7px !important; font-size:.61rem !important; line-height:1.15 !important; }
            [data-testid='stVerticalBlock'] > [data-testid='stVerticalBlock'] { gap:.12rem !important; }
            .stMarkdown { margin-bottom:.02rem !important; }
            .stMarkdown p { margin:.05rem 0 !important; }
            [data-testid='stHorizontalBlock'] { margin-bottom:.06rem !important; }
        }
        </style>
        """,
        unsafe_allow_html=True,
...
def main():
    ...
    if True:
        ...
        with right:
            ...
            setup_row_two = st.columns(2, gap="small")
            with setup_row_two[0]:
                source_kind = st.selectbox("Video အမျိုးအစား", ["Movie Recap", "Simple Movie"], key="source_kind", help="Movie Recap = နိုင်ငံခြား Recap လုပ်ပြီးသား Video · Simple Movie = ရိုးရိုး Movie/Scene Video")
            ...
            if video_duration:
                ...
                if source_kind == "Movie Recap":
                    st.caption("Recap Video ကို မူရင်းအရှည်အတိုင်း မြန်မာလိုပြန်လုပ်ပါမယ်။")
                else:
                    st.caption("Movie/Scene ကို Video အရှည်အတိုင်း မြန်မာ Voiceover/SRT လုပ်ပါမယ်။")
            else:
                duration_seconds = 0
    ...
    if True:
        ...
        audio_mix_left, audio_mix_right = st.columns(2, gap="small")
        with audio_mix_left:
        ...
        with audio_mix_right:
            ...
            if st.session_state.get("background_music_path"):
                ...
                background_music_volume = st.slider("Music အသံ", 0, 35, 12, key="background_music_volume")
            else:
        ...
        if final_video_data:
            ...
            if isinstance(final_video_data, (bytes, memoryview)):
                with st.container(border=True):
                    ...
                    st.markdown("<div class='final-result-copy'>Video ဖိုင်နဲ့ AI Thumbnail ကို ဒီနေရာမှာပဲထုတ်ပြီး ဒေါင်းပါ။</div>", unsafe_allow_html=True)
                    final_video_col, thumbnail_col = st.columns(2, gap="small")
                    with final_video_col:
                        with st.container(border=True):
                            st.markdown("<div class='final-preview-note'>Compact Preview · Final File အပြည့်အစုံ</div>", unsafe_allow_html=True)
                    ...
                    with thumbnail_col:
                        with st.container(border=True):
                            st.markdown("<div class='final-preview-note'>AI Thumbnail · Title အလိုအလျောက်</div>", unsafe_allow_html=True)
                            thumbnail_ratio = st.selectbox("Thumbnail Size", THUMBNAIL_RATIO_OPTIONS, index=1, key="thumbnail_ratio")
                            selected_part = st.selectbox("အပိုင်း", THUMBNAIL_PART_OPTIONS, key="thumbnail_part")
                            if st.button("Thumbnail ထုတ်မယ် →", type="primary", use_container_width=True, key="generate-thumbnail"):
                                story_script = str(st.session_state.get("script", "")).strip()
                                if not story_script:
                                    st.warning("Video Script မရှိသေးလို့ Thumbnail မထုတ်နိုင်ပါ။ Video ကိုအရင် Export ထုတ်ပါ။")
                                else:
                                    with st.spinner("Video အကြောင်းအရာကနေ Title နဲ့ Thumbnail ဖန်တီးနေပါတယ်..."):
                                        try:
                                            ...
                                            st.session_state.thumbnail_ratio_used = thumbnail_ratio
                                            st.session_state.thumbnail_part_used = selected_part
                                            st.rerun()
                                        except Exception as exc:
                                            st.error(f"Thumbnail မထုတ်နိုင်ပါ: {api_error_message(exc)}")
                            thumbnail_data = st.session_state.get("thumbnail_data")
                            if thumbnail_data:
                                st.caption(f"AI Title: {st.session_state.get('thumbnail_title', '')}")
                                st.image(thumbnail_data, use_container_width=True)
                                st.download_button(
                                    "Thumbnail ဒေါင်းရန်",
                                    thumbnail_data,
                                    file_name=f"movie-recap-thumbnail-{st.session_state.get('thumbnail_ratio_used', thumbnail_ratio).replace(':', 'x')}.png",
                                    mime="image/png",
