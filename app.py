...
def main():
    ...
    with st.container(border=True):
        ...
        persistent_preview = st.session_state.get("output_video") or st.session_state.get("blurred_video_path") or st.session_state.video_path
    ...
    if st.session_state.get("audio"):
        ...
        if subtitle_enabled:
            ...
            subtitle_srt = st.text_area("အချိန်ပါ SRT စာတန်းထိုး", height=120, key="subtitle_srt_editor")
            if subtitle_srt.strip():
        ...
        if st.button("Export Video + Voiceover →", use_container_width=True):
