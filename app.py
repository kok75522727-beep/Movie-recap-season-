...
def main():
    ...
    st.caption("Gemini-powered movie recap studio")
    workflow_steps = ["1 Video", "2 Script", "3 Copy Edit", "4 Blur Mask", "5 SRT"]
    ...
    for step_index, step_label in enumerate(workflow_steps, start=1):
        with step_cols[step_index - 1]:
            if st.button(step_label, key=f"workflow-step-{step_index}", use_container_width=True, type="primary" if st.session_state.workflow_step == step_index else "secondary"):
                st.session_state.workflow_step = step_index
                st.rerun()
    ...
    persistent_preview = st.session_state.get("blurred_video_path") or st.session_state.video_path
    ...
    with right:
        ...
        quality_mode = st.selectbox("Video Quality", ["720", "1280"], key="quality_mode", help="Platform က Aspect Ratio ကိုရွေးပြီး ဒီနေရာမှာ 720 သို့မဟုတ် 1280 ကိုရွေးပါမယ်။")
    ...
    if st.session_state.get("script") and active_step == 2:
        st.divider()
        st.subheader("2 · AI Script")
    ...
    if st.session_state.get("script") and active_step == 3:
        st.divider()
        st.subheader("3 · Copy Edit")
        st.caption("ဒီအဆင့်မှာ Script ကို ပြင်ပြီး နောက်အဆင့်ကို ဆက်သွားပါ။")
        copy_edit_script = st.text_area("Editable narration", value=st.session_state.script, height=230, key="copy_edit_script")
        st.session_state.script = copy_edit_script
        st.download_button("Script ဒေါင်းရန်", st.session_state.script, file_name="recap-script.txt", mime="text/plain", key="copy-edit-download")

    if st.session_state.get("script") and active_step == 4:
        st.divider()
        st.subheader("4 · Blur Mask")
        ...
        try:
            ...
            if blur_enabled:
                st.markdown("#### Anti-Copyright Effects")
                st.caption("လိုချင်တဲ့ Effect တွေကို ဖွင့်ပါ။ Final Export မှာသာ အသုံးချပါမယ်။")
                effect_left, effect_right = st.columns(2)
                with effect_left:
                    mirror_enabled = st.checkbox("Mirror (ဘယ်/ညာလှန်)", value=st.session_state.get("effect_mirror", False), key="effect_mirror")
                    color_filter_enabled = st.checkbox("Color Filter", value=st.session_state.get("effect_color_filter", False), key="effect_color_filter")
                with effect_right:
                    auto_zoom_enabled = st.checkbox("Auto Zoom", value=st.session_state.get("effect_auto_zoom", False), key="effect_auto_zoom")
                    pitch_alter_enabled = st.checkbox("Audio Pitch Alter", value=st.session_state.get("effect_pitch_alter", False), key="effect_pitch_alter")
            else:
                mirror_enabled = auto_zoom_enabled = color_filter_enabled = pitch_alter_enabled = False
            if not blur_enabled:
                st.session_state.blurred_video_path = None
                ...
                if st.button("Apply Blur Masks and Continue", type="primary", use_container_width=True):
                    with st.spinner("Blur Mask ကို Video တစ်ခုလုံးပေါ်မှာ ထည့်နေပါတယ်..."):
                        ...
                        try:
                            ...
                            st.session_state.blurred_video_path = str(apply_region_blur(st.session_state.video_path, boxes, blur_strength, background_style, solid_box_color))
                            st.session_state.audio = None
    ...
    if st.session_state.get("script") and active_step == 5:
        st.divider()
        st.subheader("4 · Blur Mask + Audio")
        ...
        if st.button("Voiceover ထုတ်မယ်", type="primary", use_container_width=True):
            with st.spinner(f"Gemini {voice} အသံနဲ့ Voiceover ပြုလုပ်နေပါတယ်..."):
                try:
                    ...
                    st.session_state.generated_srt = normalize_srt_text(script_to_srt(st.session_state.script, adjusted_duration))
    ...
    if st.session_state.get("audio"):
        ...
        subtitle_enabled = st.toggle("မြန်မာစာတန်းထိုး ဖွင့်မည်", value=st.session_state.get("subtitle_enabled", False), key="subtitle_enabled")
        if subtitle_enabled:
            st.subheader("5 · စာတန်းထိုး (SRT)")
            st.caption("Voiceover ထွက်ပြီးမှ အသံအရှည်အတိုင်း SRT timing ချပြီး စာတန်းထိုးပါမယ်။ Toggle မဖွင့်ထားရင် စာတန်းမပါဘဲ Video + Voiceover ကို ထုတ်ပါမယ်။")
            ...
            if not generated_srt and st.session_state.get("script", "").strip():
                audio_duration = max(0.1, len(st.session_state.audio) / (24000 * 2))
            ...
            st.download_button("SRT ဒေါင်းရန်", srt_download, file_name="burmese-subtitles.srt", mime="text/plain", use_container_width=True, key="download_srt_immediate")
            ...
            with st.expander("Subtitle Style / Live Preview", expanded=False):
                ...
                st.caption("စာတန်းနေရာကို X/Y နဲ့ပဲ ရွှေ့ပါ။")
                ...
                try:
                    subtitle_test_source = st.session_state.get("blurred_video_path") or st.session_state.video_path
                    ...
                    st.image(subtitle_test_image, caption="Live Subtitle Test · အသံထုတ်ပြီးမှ စမ်းကြည့်နိုင်ပါတယ်", use_container_width=True)
                except Exception as preview_exc:
                    st.info(f"Subtitle Test Preview မရသေးပါ။ Final Export မှာ Style ထည့်ပေးပါမယ်: {preview_exc}")
        ...
        if st.button("Video + Voiceover ဖိုင် ထုတ်မယ်", use_container_width=True):
            with st.spinner("Audio Speed ချိန်ပြီး Video ကို အရှည်ကိုက်အောင် Auto-fit လုပ်နေပါတယ်..."):
                try:
                    ...
                    if st.session_state.get("subtitle_enabled", False):
                        st.info(f"Video + Voiceover + SRT ကို {format_duration(round(shared_final_duration if 'shared_final_duration' in locals() else preview_duration))} အရှည်တစ်ခုတည်းနဲ့ ညှိပြီး ထုတ်နေပါတယ်။")
