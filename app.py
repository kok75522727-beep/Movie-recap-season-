...
def shift_srt_earlier(srt_text: str, lead_seconds: float = SUBTITLE_LEAD_SECONDS) -> str:
    """Move automatic captions slightly ahead of speech to compensate playback/render latency."""
    lead_ms = max(0, round(float(lead_seconds) * 1000))
    normalized = normalize_srt_text(srt_text)
    shifted: list[str] = []
    for raw_block in re.split(r"\n\s*\n", normalized):
        lines = [line.strip() for line in raw_block.splitlines() if line.strip()]
        ...
        try:
            ...
            def to_ms(value: str) -> int:
                ...
                if not match:
                    raise ValueError("Invalid SRT timestamp")
                return (((int(match.group(1)) * 60 + int(match.group(2))) * 60 + int(match.group(3))) * 1000 + int(match.group(4)))
            start_ms = max(0, to_ms(start_text) - lead_ms)
            end_ms = max(start_ms + 100, to_ms(end_text) - lead_ms)
            shifted.append(f"{len(shifted) + 1}\n{seconds_to_srt_time(start_ms / 1000)} --> {seconds_to_srt_time(end_ms / 1000)}\n{' '.join(lines[2:])}\n")
        except (TypeError, ValueError):
            continue
    return "\n".join(shifted) or normalized
...
def render_live_subtitle_browser_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, x_percent: int = 50, y_percent: int = 86, output_width: int | None = None, output_height: int | None = None) -> None:
    ...
    <style>
      ...
      .stage{{position:relative;width:100%;height:auto;aspect-ratio:{reference_width}/{reference_height};overflow:hidden;background:#111;border:1px solid rgba(34,184,255,.34);border-radius:14px;box-sizing:border-box;}}
...
def run_one_click_youtube_export(video_path: Path, duration_seconds: int, tone: str, mode: str, voice: str, voice_style: str, audio_speed: float, blur_enabled: bool, blur_masks: list[dict], blur_strength: int, blur_background: str, blur_color: str, export_style: dict, logo_path: str | None, logo_position: str, logo_motion: str, moving_logo_text: str, text_position: str, effect_auto_zoom: bool, effect_color_filter: bool, effect_pitch_alter: bool, subtitle_enabled: bool = True, progress_callback=None, timing_basis: str = "Audio အချိန်", effect_background_blur: bool = False, effect_freeze_bypass: bool = False, cached_script: str | None = None, cached_audio: bytes | None = None, target_platform: str | None = None, quality_mode: str | None = None) -> tuple[bytes, str, str, bytes]:
    ...
    generated_srt = shift_srt_earlier(script_to_srt(script, adjusted_duration))
    if progress_callback:
            ...
            with subtitle_preview_col:
                try:
                    ...
                    if subtitle_test_source:
                        ...
                        preview_output_width, preview_output_height = render_dimensions(platform, quality_mode)
                        preview_canvas = build_subtitle_preview_canvas(subtitle_test_frame, preview_output_width, preview_output_height, bool(st.session_state.get("effect_background_blur", False)))
                        render_live_subtitle_browser_preview(preview_canvas, subtitle_test_text, subtitle_font, subtitle_size, subtitle_text_color, subtitle_outline_color, subtitle_background_mode, subtitle_background_color, subtitle_background_opacity, subtitle_x, subtitle_y, preview_output_width, preview_output_height)
                    else:
        ...
        if final_video_data:
            ...
            if isinstance(final_video_data, (bytes, memoryview)):
                with st.container(border=True):
                    ...
                    final_preview_left, final_preview_center, final_preview_right = st.columns([0.18, 0.64, 0.18], gap="small")
                    with final_preview_center:
                        with st.container(border=True):
