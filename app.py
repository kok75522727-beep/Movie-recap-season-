...
def main():
    ...
    if st.session_state.get("audio") and active_step in (4, 5):
        ...
        if subtitle_enabled and active_step == 5:
            ...
            subtitle_plain_default = srt_to_plain_text(generated_srt)
            subtitle_plain = st.text_area(
                "စာတန်းထိုးမယ့်စာ",
                value=st.session_state.get("subtitle_plain_editor", subtitle_plain_default),
                height=90,
                key="subtitle_plain_editor",
                help="Video ပေါ်မှာ ပေါ်မယ့် မြန်မာစာကို ဒီနေရာမှာ ပြင်နိုင်ပါတယ်။",
            )
            rebuild_col, srt_col = st.columns(2, gap="small")
            with rebuild_col:
                if st.button("စာတန်းအချိန်ပြန်ညှိမယ်", use_container_width=True, key="rebuild-subtitle-srt"):
                    if subtitle_plain.strip():
                        audio_duration = max(0.1, len(st.session_state.audio) / (24000 * 2))
                        st.session_state.subtitle_srt_editor = normalize_srt_text(script_to_srt(subtitle_plain, audio_duration))
                        st.session_state.generated_srt = st.session_state.subtitle_srt_editor
                        st.rerun()
            with srt_col:
                st.caption("အောက်က SRT Box မှာ Timing ပါတဲ့စာကို တိုက်ရိုက်ပြင်နိုင်ပါတယ်။")
            subtitle_srt = st.text_area("အချိန်ပါ SRT စာတန်းထိုး", height=150, key="subtitle_srt_editor")
            if subtitle_srt.strip():
            ...
            with st.expander("စာတန်းထိုး Style / Live Preview", expanded=True):
