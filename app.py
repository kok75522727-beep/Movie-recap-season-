...
def resolve_myanmar_font(font_name: str | None = None) -> Path | None:
    ...
    bundled_names = ("ပြည်ထောင်စု-2.5.3_Bold.ttf", "ပြည်ထောင်စု_Bold.ttf", "ပြည်ထောင်စု_Regular.ttf")
    for directory in font_dirs:
...
def render_live_subtitle_browser_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, x_percent: int = 50, y_percent: int = 86) -> None:
    ...
    clean_text = first_srt_caption(text) if "-->" in (text or "") else " ".join((text or "").split())
    clean_text = html_lib.escape(clean_text[:220] or "မြန်မာစာတန်းထိုး စမ်းသပ်ခြင်း").replace("\n", "<br>")
