...
def script_to_srt(script: str, duration_seconds: int, lines_per_caption: int = 2) -> str:
    source_lines = [line.strip() for line in script.splitlines() if line.strip()]
    if not source_lines:
        return ""
    captions = []
    for source_line in source_lines:
        wrapped = wrap_subtitle_lines(source_line)
        for index in range(0, len(wrapped), lines_per_caption):
            captions.append("\\n".join(wrapped[index:index + lines_per_caption]))
    if not captions:
        return ""
    slot = duration_seconds / len(captions)
    entries = []
    for index, caption in enumerate(captions):
        start = index * slot
        end = duration_seconds if index == len(captions) - 1 else (index + 1) * slot
...
def main():
    ...
    if st.session_state.get("script"):
        ...
        if st.session_state.get("blurred_video_path"):
            ...
            st.markdown("#### Subtitle Test Preview")
            st.caption("ဖုန်းမှာ အလျားလိုက် မခွဲဘဲ Video အောက်မှာ စာတန်းထိုးပုံစံကို တိုက်ရိုက်စမ်းကြည့်နိုင်ပါတယ်။")
            ...
            try:
                subtitle_test_frame = extract_preview_frame(Path(st.session_state.blurred_video_path), 0)
                ...
                st.image(subtitle_test_image, caption="Live Subtitle Test · Video အောက်မှာကြည့်ပြီး Style ကို ဆက်ချိန်နိုင်ပါတယ်", use_container_width=True)
            ...
            with st.spinner("Audio Speed ချိန်ပြီး Video ကို အရှည်ကိုက်အောင် Auto-fit လုပ်နေပါတယ်..."):
                try:
                    ...
                    if st.session_state.get("blurred_video_path") and st.session_state.get("subtitle_text", "").strip():
                        ...
                        sync_target = calculate_sync_plan(source_duration, narration_duration, audio_speed)["target"]
                        subtitle_srt_to_burn = normalize_srt_text(script_to_srt(st.session_state.subtitle_text, max(1, round(sync_target))))
