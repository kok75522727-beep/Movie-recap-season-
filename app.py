...
SUBTITLE_PIPELINE_VERSION = "unicode-burmese-v4"
...
def first_srt_caption(srt_text: str) -> str:
    """Return only the first timed caption, capped at two lines, for the static style preview."""
...
def preview_caption_text(srt_text: str, max_chars: int = 18) -> str:
    """Return a compact two-line sample without changing the full SRT used for export."""
...
def render_live_subtitle_browser_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, x_percent: int = 50, y_percent: int = 86) -> None:
    ...
    clean_text = preview_caption_text(text, 18) if "-->" in (text or "") else "\n".join(wrap_subtitle_lines(" ".join((text or "").split()), 18)[:2])
    clean_text = html_lib.escape(clean_text[:120] or "မြန်မာစာတန်းထိုး စမ်းသပ်ခြင်း").replace("\n", "<br>")
    ...
    component_height = max(260, min(560, int(frame.height * 0.72)))
    html = f"""
    <style>
      {font_face}
      ...
      .caption{{position:absolute;left:{x}%;top:{y}%;transform:translate(-50%,-50%);max-width:86%;padding:8px 12px;text-align:center;color:{text_color_safe};font-family:'MgMyanmar','Noto Sans Myanmar','Pyidaungsu',sans-serif;font-size:{font_size_safe}px;line-height:1.2;white-space:normal;overflow-wrap:anywhere;text-shadow:-2px -2px 0 {outline_color_safe},2px -2px 0 {outline_color_safe},-2px 2px 0 {outline_color_safe},2px 2px 0 {outline_color_safe};z-index:2;}}
...
def merge_audio_video(video_path: Path, audio_bytes: bytes, platform: str, speed: float = 1.0, subtitle_srt: str = "", subtitle_font: str = "Noto Sans Myanmar", subtitle_size: int = 34, subtitle_text_color: str = "#FFFFFF", subtitle_outline_color: str = "#000000", subtitle_background_mode: str = "Transparent", subtitle_background_color: str = "#000000", subtitle_background_opacity: int = 55, subtitle_position: str = "Bottom", subtitle_x: int = 50, subtitle_y: int = 86, effect_mirror: bool = False, effect_auto_zoom: bool = False, effect_color_filter: bool = False, effect_pitch_alter: bool = False, logo_path: str | Path | None = None, logo_position: str = "Top right", logo_motion: str = "Static", moving_logo_text: str = "", text_position: str = "Bottom center", quality_mode: str = "720") -> bytes:
    ...
    if srt_path is not None:
        ...
        video_filter += "," + build_subtitle_filter(srt_path, subtitle_font_name, subtitle_size, subtitle_text_color, subtitle_outline_color, subtitle_background_mode, subtitle_background_color, subtitle_background_opacity, subtitle_position, subtitle_fonts_dir, subtitle_x, subtitle_y)
    if moving_logo_text.strip():
        ...
        if logo_font:
            ...
            text_x = {"Top left": "24", "Top center": "(w-tw)/2", "Top right": "w-tw-24", "Bottom left": "24", "Bottom center": "(w-tw)/2", "Bottom right": "w-tw-24", "Center": "(w-tw)/2"}.get(text_position, "(w-tw)/2")
            text_y = {"Top left": "24", "Top center": "24", "Top right": "24", "Bottom left": "h-th-36", "Bottom center": "h-th-36", "Bottom right": "h-th-36", "Center": "(h-th)/2"}.get(text_position, "h-th-36")
            if logo_motion == "Left to right":
                moving_x = "mod(t*180\\,w+tw)-tw"
            elif logo_motion == "Right to left":
                moving_x = "w-mod(t*180\\,w+tw)"
            else:
                moving_x = text_x
            moving_y = "mod(t*120\\,h+th)-th" if logo_motion == "Top to bottom" else ("h-mod(t*120\\,h+th)" if logo_motion == "Bottom to top" else text_y)
            video_filter += f",drawtext=fontfile='{escaped_logo_font}':textfile='{escaped_text}':fontcolor=white:bordercolor=black:borderw=2:fontsize=34:x={moving_x}:y={moving_y}"
    ...
    if logo_file:
        logo_x = {"Top left": "24", "Top right": "W-w-24", "Bottom left": "24", "Bottom right": "W-w-24", "Center": "(W-w)/2"}.get(logo_position, "W-w-24")
        logo_y = {"Top left": "24", "Top right": "24", "Bottom left": "H-h-24", "Bottom right": "H-h-24", "Center": "(H-h)/2"}.get(logo_position, "24")
        # The uploaded logo image always stays fixed. Motion, when selected,
        # belongs only to the separate moving text layer above.
...
def main():
    ...
    if active_step == 1:
        ...
        with right:
            ...
            if st.button("Script ရေးမယ်", type="primary", use_container_width=True):
    ...
    if active_step == 2:
        ...
        logo_upload = st.file_uploader("Logo ပုံထည့်ရန်", type=["png", "jpg", "jpeg", "webp"], key="logo_upload")
        if logo_upload and st.session_state.get("logo_upload_name") != logo_upload.name:
            st.session_state.logo_overlay_path = str(save_upload(logo_upload))
            st.session_state.logo_upload_name = logo_upload.name
        logo_button_left, logo_button_right = st.columns(2, gap="small")
        with logo_button_left:
            if st.button("Test Logo ထည့်မယ်", key="test-logo-button", use_container_width=True):
                test_logo_path = Path(__file__).resolve().parent / "test_logo.png"
                if test_logo_path.is_file():
                    st.session_state.logo_overlay_path = str(test_logo_path)
                    st.session_state.logo_upload_name = "Mg Khant Test Logo"
                else:
                    st.warning("Test Logo ဖိုင် မရသေးပါ။ Logo ပုံတစ်ခု အရင်တင်ပါ။")
        with logo_button_right:
            if st.button("Logo ဖယ်မယ်", key="clear-logo-button", use_container_width=True):
                st.session_state.pop("logo_overlay_path", None)
                st.session_state.pop("logo_upload_name", None)
        logo_position = st.selectbox("Logo နေရာ", ["Top left", "Top right", "Bottom left", "Bottom right", "Center"], key="logo_position")
        st.session_state.setdefault("logo_motion", "Static")
        logo_motion = st.selectbox("စာသားပြေးမည့်ပုံစံ", ["Static", "Left to right", "Right to left", "Top to bottom", "Bottom to top"], key="logo_motion", help="Static ရွေးရင် စာငြိမ်နေမယ်။ ပြေးတဲ့လမ်းတစ်ခုရွေးမှသာ စာပြေးမယ်။")
        moving_logo_text = st.text_input("Logo စာသား (မလိုရင် အလွတ်ထားပါ)", key="moving_logo_text", placeholder="ဥပမာ - Mg Khant")
        text_position = st.selectbox("စာသားနေရာ", ["Top left", "Top center", "Top right", "Bottom left", "Bottom center", "Bottom right", "Center"], key="text_position")
    ...
    if active_step == 3:
        ...
        try:
            ...
            if blur_enabled:
                ...
                if st.button("Blur မလုပ်ဘဲ Voiceover သို့ ဆက်မယ် →", key="skip-blur-step", use_container_width=True):
                    st.session_state.blur_enabled = False
                    st.session_state.blurred_video_path = None
                    st.session_state.workflow_step = 4
                    st.rerun()
    ...
    if active_step == 4:
        ...
        with voice_row[1]:
            voice = st.selectbox("Voice", list(VOICE_OPTIONS.keys()), format_func=lambda item: f"{item} · {VOICE_OPTIONS[item]}")
        with voice_row[2]:
    ...
    if active_step == 5 or (st.session_state.get("audio") and active_step == 4):
        ...
        if active_step == 4:
            ...
            if raw_audio and st.button("Video + အသံ ပေါင်းပြီး ဒေါင်းရန် →", key="export-audio-video", type="primary", use_container_width=True):
                with st.spinner("Video နဲ့ အသံကို ပေါင်းနေပါတယ်..."):
                    try:
                        ...
                        st.session_state.audio_video_output = merge_audio_video(
                            ...
                            logo_motion=st.session_state.get("logo_motion", "Static"),
        ...
        if subtitle_enabled and active_step == 5:
            ...
            with subtitle_controls:
                ...
                # Keep an explicit export snapshot. This avoids a stale widget
                # value when the user changes Font and immediately presses Export.
