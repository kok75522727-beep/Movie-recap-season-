...
def render_live_subtitle_preview(frame: Image.Image, text: str, font_name: str, font_size: int, text_color: str, outline_color: str, background_mode: str, background_color: str, background_opacity: int, position: str) -> Image.Image:
    ...
    if position == "Top":
        ...
        start_y = max(12, (image.height - block_height) // 2)
    else:
        start_y = max(12, image.height - block_height - 34)
    if background_mode == "Solid background":
        pad_x, pad_y = 16, 10
        max_line_width = max((box[2] - box[0] for box in line_boxes), default=0)
        bg_box = (max(8, (image.width - max_line_width) // 2 - pad_x), max(4, start_y - pad_y), min(image.width - 8, (image.width + max_line_width) // 2 + pad_x), min(image.height - 4, start_y + block_height + pad_y))
...
def merge_audio_video(video_path: Path, audio_bytes: bytes, platform: str, speed: float = 1.0, subtitle_srt: str = "", subtitle_font: str = "Noto Sans Myanmar", subtitle_size: int = 34, subtitle_text_color: str = "#FFFFFF", subtitle_background_mode: str = "Transparent", subtitle_background_color: str = "#000000", subtitle_background_opacity: int = 55, subtitle_position: str = "Bottom", quality_mode: str = "720") -> bytes:
    ...
    video_filter = f"setpts=PTS/{auto_video_speed:.6f},scale={output_width}:{output_height}:force_original_aspect_ratio=increase,crop={output_width}:{output_height}"
    if subtitle_srt.strip():
    ...
    if st.session_state.get("script"):
        ...
        if st.session_state.get("blurred_video_path"):
            ...
            try:
                ...
                subtitle_test_image = render_live_subtitle_preview(subtitle_test_frame, subtitle_text, subtitle_font, subtitle_size, subtitle_text_color, subtitle_outline_color, subtitle_background_mode, subtitle_background_color, subtitle_background_opacity, subtitle_position)
