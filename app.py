...
def main():
    ...
    with st.container(border=True):
        ...
        workflow_steps = ["1 Script", "2 Copyright Edit", "3 Blue Mask", "4 စာတန်းထိုး"]
        ...
        with skip_col:
            if st.button("ကျော်မယ် →", key="workflow-skip", use_container_width=True):
                st.session_state.workflow_step = min(4, active_step + 1)
                st.rerun()
    ...
    if st.session_state.get("script") and active_step == 1:
        st.divider()
    ...
    if st.session_state.get("script") and active_step == 2:
        ...
        effect_steps = [
            ...
            ("effect_color_filter", "Color Filter"),
            ("effect_pitch_alter", "Audio Pitch Alter"),
        ]
        ...
        for position, (effect_key, effect_label) in enumerate(effect_steps):
            ...
            with target_col:
                ...
                if st.button(button_text, key=f"effect-group-{effect_key}", use_container_width=True):
                    ...
                    st.rerun()
        st.caption("Effect တစ်ခုကို နှိပ်ပြီး ရွေး/မရွေး ပြောင်းနိုင်ပါတယ်။")
        if st.button("အုပ်စု ၂ ပြီးပြီ · အုပ်စု ၃ သို့ →", key="effect-group-next", type="primary", use_container_width=True):
    ...
    if st.session_state.get("script") and active_step == 4:
        st.divider()
        st.subheader("4 · စာတန်းထိုး + Audio")
