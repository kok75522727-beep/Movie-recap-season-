...
def main():
    ...
    if st.session_state.get("audio"):
        ...
        if subtitle_enabled:
            ...
            generated_srt = st.session_state.get("generated_srt", "")
            subtitle_srt = st.text_area("အချိန်ပါ SRT စာတန်းထိုး", value=st.session_state.get("subtitle_srt_editor", generated_srt), height=180, key="subtitle_srt_editor")
            st.session_state.generated_srt = normalize_srt_text(subtitle_srt)
            st.download_button("SRT ဒေါင်းရန်", st.session_state.generated_srt.encode("utf-8"), file_name="burmese-subtitles.srt", mime="text/plain", use_container_width=True, key="download_srt_immediate")
            st.success("Voiceover နဲ့ SRT ကို တစ်ခါတည်းဖန်တီးထားပြီးပါပြီ · Timing ပါပြီးသား · အများဆုံး ၂ ကြောင်း")
            subtitle_text = srt_to_plain_text(st.session_state.generated_srt)
